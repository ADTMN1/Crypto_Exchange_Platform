...
export default function OrdersPage() {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await adminService.getAllOrders({ status: statusFilter || undefined });
      if (response.success) {
  ...
  return (
    <main className="nex-admin-section-page">
      <section className="nex-section-header">
        <div>
          <h1>All Orders</h1>
          <p>View and manage all orders on the platform</p>
        </div>
      </section>
          ...
          <div className="nex-card-title">
            ...
            <div style={{ display: 'flex', gap: '1rem' }}>
              ...
              <select
                ...
                style={{ padding: '0.5rem', borderRadius: '0.5rem', border: '1px solid rgba(169,255,232,0.2)', background: 'transparent', color: 'var(--text-primary)' }}
              >
                ...
                <tbody>
                  {filteredOrders.length === 0 ? (
                    ...
                    filteredOrders.map((order) => {
                      ...
                      const isCancelable = ['open', 'partially_filled'].includes(order.status);
